...
const MusicPlayer: React.FC<MusicPlayerProps> = ({ initialData }) => {
  ...
  return (
    <div className="relative min-h-screen px-4">
      ...
      {/* Music Image for desktop and mobile */}
      {currentSong && currentSong.imgUrl && (
        <div className="flex items-center justify-center h-auto w-full max-w-full lg:max-w-[1400px]">
          <CustomImage
            src={currentSong.imgUrl}
            alt={currentMusicName || "an image associated with the music"}
            dataUrl={currentTrack}
      ...
      {/* Video and Mood Message */}
      {isVideoVisible && (
        <div
          className={`relative ${
            isVideoVisible ? "block" : "hidden"
          } sm:fixed sm:bottom-8 sm:right-8 sm:w-80 sm:h-80 flex flex-col items-center justify-center w-full`}
        ...
        >
          <video
            ...
            muted
            playsInline
            className="w-full h-48 sm:h-auto sm:w-full object-cover sm:object-center"
          />
          <div className="absolute top-0 left-0 right-0 text-center p-4 bg-opacity-75 bg-black text-white font-bold">
            {getMoodMessage()}
          </div>
